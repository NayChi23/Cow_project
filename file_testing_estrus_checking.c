...
int main() {
    long int numberZero =0; //only reset to 0 if the data are equal to one day
    long int numberOne =0;  //Store how many 1's in the data
    //unsigned int num=0;
    bool FirstTurn = 1;
    ...
    char path[] = "C:/Users/mcc/Desktop/Internship/cow_file/text_file";
    char filenames[MAX_FILES][MAX_FILENAME_LENGTH];
    char individual_file[MAX_FILENAME_LENGTH];
    //char lines[MAX_LINES][MAX_FILENAME_LENGTH];
    char lines[MAX_FILENAME_LENGTH];
    //int file_count = 0;
    struct dirent *entry;
    DIR *dp;
    ...
    //247
    // Read and display the filenames in the directory
    ...
    int result;
    for(int i =0;i<31;i++){
    // Print individual characters for clarity
    printf("Individual characters: %c %c\n", num[0], num[1]);
    dp = opendir(path);
       int file_count = 0;
        while ((entry = readdir(dp)) != NULL) {
        if (entry->d_type == DT_REG) { // Check if it's a regular file
            strncpy(individual_file, entry->d_name, MAX_FILENAME_LENGTH);
            if(individual_file[0] == num[0] && individual_file[1] == num[1]){
                strncpy(filenames[file_count], entry->d_name, MAX_FILENAME_LENGTH);
                replace_last_txt(filenames[file_count]);
                filenames[file_count][MAX_FILENAME_LENGTH - 1] = '\0'; // Ensure null termination
                file_count++;  
            } 
            if (file_count >= MAX_FILES) {
                break; // Stop if we reach the maximum number of files we can store
            }
        }
    }
    result = atoi(num);
	...
	if(file_count == 0)
	{
		printf("The loop pass\n");
		continue;
	}
    closedir(dp);
    
    // Print the filenames stored in the array
    //printf("Files in the directory '%s':\n", path);
    ...
    //}
        ...
        printf("this is file no %d\n",i);
        char tempoarray[MAX_COLS][MAX_FILENAME_LENGTH];
        if (file == NULL) {
            perror("Error opening file");
            return EXIT_FAILURE;
        }
        // Read lines into array
        while (fgets(lines, MAX_FILENAME_LENGTH, file)) {
        int col = 0;
    ...
    Ccount += 1;
    if(FirstTurn == 1 && Ccount == 60)
    {
        Ccount = 0;
        enqueue(total_one_day_count,numberOne);
        printf("the total number one is %ld\n", numberOne);
        numberOne = 0;
        numberZero = 0;
        check_First_two_day += 1;
        printf("Complete %d turn \n", check_First_two_day);
        printQueue(total_one_day_count);
        if(check_First_two_day == 15)
            {
                FirstTurn = 0;
                printf("The result of compared data is\t%d\n",CompareData(total_one_day_count));
                if(CompareData(total_one_day_count) != -1){
                    insertAtEnd(&result_data, CompareData(total_one_day_count));
                }
                printList(result_data);
            }

    }
    else if(FirstTurn  == 0 && Ccount == 60){
        Ccount = 0;
        printf("the total number one is %ld\n", numberOne);
